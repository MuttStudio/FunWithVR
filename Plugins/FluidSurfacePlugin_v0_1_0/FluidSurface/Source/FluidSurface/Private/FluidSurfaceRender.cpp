...
, MaterialRelevance(Component->GetMaterialRelevance(GRHIFeatureLevel))
, Time(0.0f)
, LevelColor(1, 1, 1)
, PropertyColor(1, 1, 1)
...
void FFluidSurfaceSceneProxy::DrawDynamicElements(FPrimitiveDrawInterface* PDI, const FSceneView* View)
{
    ...
    if (bDrawMesh)
    {
        ...
        // Editor depts testing - not sure this is right - RUSS
    ...
    if ((bDrawSimpleCollision) && AllowDebugViewmodes())
    {
		...
		// Editor depth testing - not sure this is the right change - RUSS
...
void FFluidSurfaceSceneProxy::ExecComputeShader()
{
    ...
    // Global shader map - not sure if this is right - RUSS
    TShaderMapRef<FFluidSurfaceCS> FluidSurfaceCS(GetGlobalShaderMap(GRHIFeatureLevel, false));
